...
const GridCard = ({ item }: Props) => {
  ...
  const { setCurrentBackGround } = useContext(GridBackgroundContext);
  const [copied, setCopied] = useState<boolean>(false);
  const [applied, setApplied] = useState<boolean>(false);

  const handleApply = (background: GridBackground) => {
  ...
  const handleCopy = (background: GridBackground) => {
    try {
    ...
    >
      ...
      <div className="absolute left-0 bottom-0 right-0 backdrop-blur-sm p-2 z-20 ">
        <div className="w-full flex items-center flex-col gap-2">
          ...
          <Button
            onClick={(e) => {
              e.stopPropagation(); // Prevent Apply on Copy
              handleCopy(item);
            }}
            className={`text-xs w-full text-center truncate cursor-pointer ${
              item.theme === "dark"
        ...
        <p
          className={`text-sm font-medium text-center truncate mt-4 ${
